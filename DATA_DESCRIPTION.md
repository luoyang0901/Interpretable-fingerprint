...
Radius-2 in-house/Gao alignment plus exact overlap, near-similarity, representation-collision, and FPtand encoding audits. The `*_safe_any_encoding_group_ids.npy` arrays are the conservative groups used for TextCNN splitting.
...
Complete leakage-controlled SHAP archive for the pre-specified radius-3 Random Forest, including all 2,048 feature importance values, test-sample SHAP values, pre-defined top ten features, correlations, mapped molecular environments, and the interpretation manifest.
